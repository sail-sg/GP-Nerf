...
class Renderer(nn.Module):
    def __init__(
        ...
        self.code = None
        self.rays_num = 0
        self.sigma_pts_num = 0
        self.rgb_pts_num = 0
        self.pts_num = 0
        self.encoder_time = 0
        self.loader_time = 0
        self.sigma_full_time = 0
        # self.ray_time = 0
        self.sigma_time = 0
        self.rgb_full_time = 0
        self.rgb_time = 0
        # self.render_time = 0
        self.full_render_time = 0
    ...
    def render_rays(self, neg_ray, featmaps, src_imgs, src_cameras, batch):
        ...
        # torch.cuda.synchronize()
        # self.sptime += time.time() - start_time
        ...
        # mask_at_box = batch['mask_at_box'][0].clone()
        ...
        # new_ray_idx = (torch.cat([new_ray_idx1, new_ray_idx2, new_ray_idx3, new_ray_idx4], dim=0)).long()
        new_mask_at_box = torch.zeros((W * W)).to("cuda") == 0
        # new_mask_at_box[new_ray_idx] = 1.0
        i, j = torch.meshgrid(torch.arange(W).float(), torch.arange(W).float())
        # j, i = torch.where(new_mask_at_box.view(W, -1)==1)
        xy1 = torch.stack([j, i, torch.ones_like(i)], dim=-1).to("cuda").view(-1, 3)
        ...
        pixel_world = (pixel_camera - target_pose[0, :, 3:].T) @ target_pose[0, :, :3]
        ...
        nominator = can_bounds - rays_o[:, None]
        ...
        far = torch.max(d0, d1)
        # chunk_size = len(rays_o) // 3
        self.chunk_i = 2
        chunk_size = len(rays_o) // self.chunk_i

        self.rays_num += len(rays_o)
        rays_o_list = torch.split(rays_o, chunk_size)
        rays_d_list = torch.split(rays_d, chunk_size)
        near_list = torch.split(near, chunk_size)
        far_list = torch.split(far, chunk_size)
        new_mask_at_box = mask_at_box
        out_mask_at_box = np.zeros((int(W), int(W))) == 1

        torch.cuda.synchronize()
        self.sigma_full_time += time.time() - ts_time

        for chunk_i, (rays_o, rays_d, near, far) in enumerate(
            zip(rays_o_list, rays_d_list, near_list, far_list)
        ):
            torch.cuda.synchronize()
            ts_time = time.time()
            ...
            self.pts_num += len(pts.squeeze(0))
            # sp_feats = F.grid_sample(feat.unsqueeze(0).unsqueeze(0),
            ...
            torch.cuda.synchronize()
            start_time = time.time()
            _, sigma = self.nerfhead.sigmahead(
                sp_input, grid_coords, rgb_feat, mask
            )  # update
            torch.cuda.synchronize()
            self.sigma_time += time.time() - start_time
            self.sigma_full_time += time.time() - ts_time
            ...
            # valid1 = torch.where(alpha[..., 0]>-1e5)[0]
            # rgb_feat = rgb_feat[valid1]
            torch.cuda.synchronize()
            start_time = time.time()
            raw, _ = self.nerfhead(rgb_feat)
            torch.cuda.synchronize()
            self.rgb_time += time.time() - start_time
            self.rgb_full_time += time.time() - gs_time
            ...
            chunk_len = min(len(xy1), chunk_size)
            cur_new_mask = torch.zeros((W * W)) == 1
            cur_new_mask[
                torch.where(new_mask_at_box == 1)[0][start : start + chunk_len]
            ] = 1
            mask_at_box = cur_new_mask.reshape((int(W), int(W))).data.cpu().numpy() == 1
            out_rgb_map.append(rgb_map)
            pred_img[mask_at_box] = rgb_map
            out_mask_at_box = out_mask_at_box + mask_at_box
            # torch.cuda.synchronize()
            # self.render_time += time.time() - start_time1

        # cat rgb_map
        rgb_map = np.vstack(out_rgb_map)
        mask_at_box = out_mask_at_box > 0
        ret = {
            "rgb_map": rgb_map,
            "pred_img": pred_img,
    ...
    def render(self, batch):
        ...
        # print(f"get featmaps: {featmaps.shape}")
        # _ = input()
        torch.cuda.synchronize()
        ...
        if self.nerfhead.use_rgbhead:
            ...
            torch.cuda.synchronize()
            self.loader_time += time.time() - start_time
            ...
            torch.cuda.synchronize()
            self.full_render_time += time.time() - rs_time
            ...
            ret = self.batchify_rays(
                pts, featmaps, src_imgs, src_cameras, self.chunk, batch
            )
            sigma = ret["sigma"][:, 0, 0]
...
class Projector:
    ...
    def compute_(self, xyz, smpl_xyz, train_imgs, train_cameras, featmaps):
        """
        :param xyz: [n_rays, n_samples, 3]
        :param train_imgs: [1, n_views, 3, h, w]
        :param train_cameras: [1, n_views, 34]
...
def build_demo_render(cfg):
    ...
    if "thuman" in cfg.dataset.train.name:
        ...
        neg_ray_train = False
    if "thuman" in cfg.dataset.test.name:  # demo data organized as zju dataset
        neg_ray_val = True
    else:
        neg_ray_val = False
    ...
    render_config = {
        ...
        "mesh_th": 1.0 / cfg.dataset.mesh_th,
    }
